...
2. Malformed prompt — a specific stable token has event data exceeding the LLM's context window. Skip enrichment for that token.
...
mongosh "mongodb://localhost:27017/windoh" --eval '
  db.tokens.findOne({"stable_token": "<hash>"})
'
