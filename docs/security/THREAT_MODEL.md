...
**Mitigations:**
- Enrichment runs exactly once per base token. If bad enrichment is stored, the operator can delete the token document and re-trigger enrichment.
...
- BullMQ jobs contain only base token references, not the full event data. Event data is read from MongoDB.
