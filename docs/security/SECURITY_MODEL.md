...
Fields excluded from encryption (needed for SQLite indexing): `id`, `timestamp`, `event_type`, base token.
...
**Fields included in exported events:** base token, payload token, `event_type`, `decay_score`, `rarity_band`, `behavior_tags`, `inter_event_delta_ms`, `tree_depth`, `ancestor_chain_hash`, `field_completeness_score`
...
- Events contain base token and payload token — not raw command lines or IPs directly
