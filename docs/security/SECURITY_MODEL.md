...
Fields excluded from encryption (needed for SQLite indexing): `id`, `timestamp`, `event_type`, stable token.
...
**Fields included in exported events:** stable token, payload token, `event_type`, `decay_score`, `rarity_band`, `behavior_tags`, `inter_event_delta_ms`, `tree_depth`, `ancestor_chain_hash`, `field_completeness_score`

**Fields NOT exported:** Raw command lines, raw IP addresses, raw file paths, user account names. These are available in the `exemplars` index (one per behavioral pattern, not one per event) or via the payload token which can be correlated with external data.
...
- Events contain stable token and payload token — not raw command lines or IPs directly
