...
- **Cross-system compatibility:** A `stable_hash` can be queried by any SQL database, any SIEM, any log aggregator. Embeddings require specialized vector database infrastructure.
...
- The `stable_hash` generation pipeline must be exhaustively deterministic — same behavior on any host at any time must produce the same hash. This requires careful exclusion of non-deterministic fields (timestamps, PIDs, handles).
- Payload-level variation tracking requires a separate `payload_hash` mechanism, since the behavioral hash intentionally ignores command-line arguments and network targets.
