...
- **Cross-system compatibility:** A base token can be queried by any SQL database, any SIEM, any log aggregator. Embeddings require specialized vector database infrastructure.
...
- The base token generation pipeline must be exhaustively deterministic — same behavior on any host at any time must produce the same hash. This requires careful exclusion of non-deterministic fields (timestamps, PIDs, handles).
