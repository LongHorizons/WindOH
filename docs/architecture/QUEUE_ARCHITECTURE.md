...
**Job data minimization:** Enrichment jobs contain only the `stable_hash` — not the full event data. The worker reads event data from MongoDB at execution time. This avoids storing potentially large event documents in Redis.
...
ES Poll Loop
    │
    ▼
New token detected (stable_hash not in tokens collection)
    │
    ▼
API enqueues job: { stable_hash: "abc123..." }
    │
    ▼
Worker dequeues job
    │
